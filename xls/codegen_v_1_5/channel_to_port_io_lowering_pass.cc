...
// Capacity: 1
//
// Allows combinational valid/data pass-through (zero latency).  When occupied,
// downstream ready propagates upstream so that a pop and push can replace the
// buffered element in one cycle.
//
...
absl::Status AddZeroLatencyBufferToRDVNodes(Node* from_data, Node* from_valid,
  ...
  XLS_ASSIGN_OR_RETURN(
      ...
      block->MakeNodeWithName<NaryOp>(
          ...
          Op::kOr, absl::StrCat(name_prefix, "_from_skid_rdy")));
  if (has_ready) {
